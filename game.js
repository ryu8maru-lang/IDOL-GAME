...
const SAVE='idol_dynasty_v014_office_home';
...
function executeWeek(){
 ...
 advanceWeek();updateLifeWeekly(g);checkLifeCallbacks(g);if(!state.pendingStory)maybeCreateStory(g);state.screen=state.pendingStory?'game':'result';render()
}
...
function resolveStory(i){
 ...
 if(main){
  ...
  if(/考える|次|前に出|センター|選抜/.test(c[0]+' '+c[1])&&e.title.includes(main.name))promiseTo(main,c[0],rand(5,10))
 }
 ...
 state.storyOutcome={groupId:g.id,title:e.title,choice:c[0],sub:c[1],effects:effectSummary(c[2],ps,g),reaction,people:e.people,lifeType:main?.lifeType||null};
...
function auditionHub(){
 const g=currentGroup(),capacity=12-members(g).length;
 return `<div class="app-page office-page">
  <div class="page-hero"><div><div class="eyebrow">AUDITION / TALENT SCOUT</div><h1>オーディション</h1><p>未来のスターを見つけよう。</p></div><div class="hero-badge">${members(g).length}<small>/ 12名</small></div></div>
  <div class="office-panel audition-spotlight"><div><span class="pink-ribbon">WEEKLY AUDITION</span><h2>追加メンバー募集</h2><p>性格、過去、夢、能力が異なる候補者が登場。</p><div class="audition-faces">${Array.from({length:5},(_,i)=>`<img src="${CANDIDATE_IMAGES[(i+state.week)%CANDIDATE_IMAGES.length]}">`).join('')}</div></div><button onclick="addMemberAudition()" ${capacity<=0?'disabled':''}>${capacity>0?'オーディション開始 →':'定員12名'}</button></div>
  <div class="office-grid-2"><div class="office-panel office-inner"><div class="panel-title">現在のグループ</div>${bigLine('グループ',g.name)}${bigLine('メンバー',members(g).length+'人')}${bigLine('世代',g.generation+'期')}${bigLine('センター',center(g)?.name||'-')}</div>
  <div class="office-panel office-inner"><div class="panel-title">加入の影響</div><p class="office-copy">新加入は友情、嫉妬、ライバル関係にも影響する。</p><button class="wide-secondary" onclick="setTab('members')">メンバー構成を見る</button></div></div>
 </div>`
}
...
function topbar(){
 return `<header class="office-topbar">
  <div class="office-brand"><div class="brand-crown">♔</div><div><strong>IDOL DYNASTY</strong><small>${esc(state.agency)} / ${esc(state.president)}社長</small></div></div>
  <div class="top-resource"><span>資金</span><b>${money(state.cash)}</b></div><div class="top-resource"><span>事務所</span><b>${office().name}</b></div><div class="top-resource"><span>評判</span><b>${state.reputation}</b></div>
  <div class="top-week"><b>${state.year}年 ${state.month}月</b><small>第${state.week}週</small></div><div class="top-icons"><button onclick="save()">💾</button><button onclick="reset()">☰</button></div>
 </header>`
}

function nav(){
 const a=[['board','⌂','ホーム'],['members','♟','メンバー'],['auditionHub','★','オーディション'],['live','🎤','ライブ'],['world','▥','事務所']];
 return `<div class="office-nav">${a.map(x=>`<button class="${state.tab===x[0]?'active':''}" onclick="setTab('${x[0]}')"><span class="ni">${x[1]}</span><span>${x[2]}</span></button>`).join('')}</div>`
}
...
function boardTab(){
 const g=currentGroup(),prog=clamp(Math.round(g.mission.current/g.mission.target*100),0,100);
 return `<main class="office-home"><div class="office-background-layer"></div>
  <section class="office-welcome"><div><span>${state.agency}</span><h1>${state.president}社長、おかえりなさい。</h1><p>今日も、アイドルたちの未来を決める一週間が始まる。</p></div><div class="office-date-orb"><strong>${state.month}</strong><span>月</span><small>第${state.week}週</small></div></section>
  <section class="office-dashboard"><div class="dashboard-left">${featuredGroupCard(g)}${officeQuickActions(g)}</div><div class="dashboard-right">${weeklyOfficeTasks(g)}${officeConsultationCard(g)}${officeNews()}</div></section>
  <section class="season-goal office-panel"><div><div class="panel-kicker">✦ メイン目標</div><h2>${g.mission.title}</h2><p>${g.mission.current.toLocaleString()} / ${g.mission.target.toLocaleString()}</p></div><div class="season-progress"><div><i style="width:${prog}%"></i></div><strong>${prog}%</strong></div><button onclick="setTab('live')">目標へ進む →</button></section>
 </main>`
}
...
function membersTab(){
 const g=currentGroup(),ms=members(g),c=center(g);
 return `<div class="app-page office-page"><div class="page-hero"><div><div class="eyebrow">MEMBERS / ${g.name}</div><h1>メンバー</h1><p>感情、記憶、関係性まで見る。</p></div><button onclick="addMemberAudition()">＋ 追加オーディション</button></div>
 ${c?`<div class="office-panel center-feature"><div>${characterFigure(c,g,'happy',false,'member-hero-char')}</div><div><span class="pink-ribbon">CENTER</span><h2>${c.name}</h2><div class="center-stat-grid"><span>人気<b>${c.popularity}</b></span><span>信頼<b>${c.trustToPresident}</b></span><span>気分<b>${c.mood}</b></span><span>疲労<b>${c.fatigue}</b></span></div><p>「${esc(profileQuote(c))}」</p><button onclick="openMember('${c.id}')">詳細プロフィール →</button></div></div>`:''}
 <div class="panel-title standalone">メンバー一覧</div><div class="member-grid app-member-grid">${ms.map(m=>memberCard(m,g)).join('')}</div><div class="panel-title standalone">メンバー関係</div><div class="relationships">${relationshipCards(g)}</div></div>`
}
...
function agencyTab(){
 ...
 return `<div class="app-page office-page"><div class="page-hero"><div><div class="eyebrow">AGENCY MANAGEMENT</div><h1>事務所</h1><p>${state.agency}そのものを育てる。</p></div>${state.groups.length<office().slots?'<button onclick="startNewGroupSetup()">＋ 新グループ</button>':''}</div>
 <div class="office-summary-grid"><div class="office-panel office-stat-card"><span>総ファン</span><strong>${totalFans().toLocaleString()}</strong><small>${state.groups.length}グループ</small></div><div class="office-panel office-stat-card"><span>評判</span><strong>${state.reputation}</strong><small>${office().name}</small></div><div class="office-panel office-stat-card"><span>資金</span><strong>${money(state.cash)}</strong><small>固定費 ${money(monthlyFixed())}</small></div></div>
 <div class="office-grid-2"><div class="office-panel office-inner"><div class="panel-title">所属グループ</div>${state.groups.map(g=>`<div class="agency-group-row"><div><strong>${g.name}</strong><small>${themeInfo(g.costumeTheme).name}</small></div><span>P ${groupPower(g)}</span><span>${g.fans.toLocaleString()}人</span><button onclick="switchGroup('${g.id}')">運営</button></div>`).join('')}</div>
 <div class="office-panel office-inner"><div class="panel-title">事務所成長</div>${bigLine('社長',`${state.president}社長`)}${bigLine('現在',office().name)}${bigLine('所属枠',`${state.groups.length}/${office().slots}組`)}${next?`<div class="upgrade-box"><strong>次：${next.name}</strong><small>必要評判 ${next.needRep} / 費用 ${money(next.cost)}</small><button onclick="upgradeOffice()">オフィス拡張</button></div>`:'<div class="tag gold" style="display:inline-block;margin:12px">最大規模</div>'}</div></div>
 <div class="panel-title standalone">スタッフ</div><div class="staff-grid">${STAFF.map(s=>`<div class="staff"><strong>${s.name}</strong><small>${s.effect}<br>月給 ${money(s.cost)}</small>${hasStaff(s.id)?`<button class="secondary" onclick="fireStaff('${s.id}')">契約中</button>`:`<button onclick="hire('${s.id}')">採用</button>`}</div>`).join('')}</div>
 <div class="panel-title standalone">アイドル業界勢力図</div><div class="office-panel office-inner">${[...state.rivals,...state.groups.map(g=>({name:g.name,power:groupPower(g),fans:g.fans,self:true}))].sort((a,b)=>b.power-a.power).map((r,i)=>`<div class="rank-row ${r.self?'self':''}"><strong>#${i+1}</strong><strong>${r.self?'★ ':''}${r.name}</strong><span>P ${r.power}</span><span>${r.fans.toLocaleString()}</span></div>`).join('')}</div></div>`
}
...
function storyResultScreen(){
 ...
 const g=groupById(o.groupId),ps=(o.people||[]).map(id=>member(id,g)).filter(Boolean),m=ps[0],emotionNow=emotionFromChoice([o.choice,o.sub]);
 return `<div class="app"><div class="result story-result cinematic-result life-result"><div class="eyebrow">DECISION RESULT / ${g.name}</div><h1>${esc(o.choice)}</h1>
 <div class="result-character-wrap">${m?characterFigure(m,g,emotionNow,false,'result-character'):''}<div class="reaction-copy"><div class="reaction-name">${m?`${m.name} / ${archetypeOf(m).name}`:''}</div><strong>「${esc(o.reaction)}」</strong><p>${esc(o.sub)}</p>${m?`<div class="after-emotions">今の気持ち：喜び ${emotion(m,'joy')} / 不安 ${emotion(m,'anxiety')} / 嫉妬 ${emotion(m,'jealousy')} / 寂しさ ${emotion(m,'loneliness')}</div>`:''}</div></div>
 <div class="effect-result"><span>この判断で変化したもの</span><strong>${esc(o.effects||'数値変化なし')}</strong></div>
 <div class="memory-note">🧠 この会話は本人の「記憶」に保存された。数週間後、別の場面でこの判断が戻ってくることがある。</div>
