...
const SAVE='idol_dynasty_v011_human_drama';
...
function memberModal(id){
 ...
 return `<div class="modal-bg" onclick="if(event.target===this)closeModal()"><div class="modal"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px"><div class="eyebrow">IDOL PROFILE / ${g.name}</div><button class="ghost" onclick="closeModal()">✕</button></div>
 <div class="profile-head"><img src="${displayImage(m,g)}"><div><h2><span style="color:${m.color}">●</span> ${m.name}</h2><div style="font-size:11px;color:var(--muted)">${m.age}歳 / ${m.hometown||'-'} / ${m.trait} / ${m.gen}期生</div><div class="traits"><span class="tag">${m.visualType}</span><span class="tag">スター ${m.star}</span><span class="tag">野心 ${m.ambition}</span><span class="tag">自信 ${m.confidence}</span></div><div class="quote">「${profileQuote(m)}」</div>${m.id!==g.centerId?`<button style="margin-top:9px" onclick="setCenter('${m.id}');closeModal()">センターに指名</button>`:'<span class="tag gold" style="display:inline-block;margin-top:9px">CURRENT CENTER</span>'}</div></div>
...
function storyModal(e){
 const g=groupById(e.groupId),ps=e.people.map(id=>member(id,g)).filter(Boolean),speaker=ps[0],boss=state.president;
 return `<div class="modal-bg"><div class="modal story-event chat-story">
 <div class="eyebrow">PRESIDENT STORY / ${g.name}</div><h2>${esc(e.title)}</h2>
 <div class="chat-thread">
   ${speaker?`<div class="chat-row idol"><img src="${displayImage(speaker,g)}"><div><small>${speaker.name}</small><div class="bubble">${esc(e.text)}</div></div></div>`:''}
   <div class="chat-row president"><div><small>${boss}社長</small><div class="bubble">どう答える？ この判断はメンバーの感情やグループの空気に残る。</div></div><div class="president-avatar">社</div></div>
 </div>
...
function storyResultScreen(){
 ...
 const g=groupById(o.groupId),ps=(o.people||[]).map(id=>member(id,g)).filter(Boolean);
 return `<div class="app"><div class="result story-result"><div class="eyebrow">DECISION RESULT / ${g.name}</div><h1>${esc(o.choice)}</h1>
 <div class="reaction-panel">${ps[0]?`<img src="${displayImage(ps[0],g)}">`:''}<div><strong>${esc(o.reaction)}</strong><p>${esc(o.sub)}</p></div></div>
