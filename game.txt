...
const SAVE='idol_dynasty_v013_life_system';
...
function applyEffects(effects,ps,g){
 ...
 if(ps?.[0]){
  ...
  if((e.p1_mood||0)>0){modEmotion(lm,'joy',Math.ceil(e.p1_mood/2));modEmotion(lm,'anxiety',-2);lm.affectionToPresident=clamp(lm.affectionToPresident+2,0,100)}
  if((e.p1_mood||0)<0){modEmotion(lm,'anxiety',Math.ceil(Math.abs(e.p1_mood)/2));modEmotion(lm,'loneliness',2)}
  if((e.p1_confidence||0)>0)lm.respectToPresident=clamp(lm.respectToPresident+1,0,100)
 }
 if(ps?.[1]){
  const lm=ps[1];ensureLife(lm);
  if((e.p2_mood||0)>0)modEmotion(lm,'joy',Math.ceil(e.p2_mood/2));
  if((e.p2_mood||0)<0)modEmotion(lm,'anxiety',Math.ceil(Math.abs(e.p2_mood)/2))
 }
}
