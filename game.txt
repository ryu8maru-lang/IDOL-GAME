...
const SAVE='idol_dynasty_v015_sky_ui';
...
function freshState(){
 return {
  ...
  liveRun:null,lastResult:null,lastLiveResult:null,liveEmotion:null,skyPanel:null
 }
}
...
function auditionHub(){
 return skyFrame('audition.png',`
<button class="hot hit-menu" onclick="openSkyPanel('menu')" aria-label="メニュー"></button>
<button class="hot hit-audition-main" onclick="addMemberAudition()" aria-label="オーディション開始"></button>
<button class="hot hit-audition-list" onclick="addMemberAudition()" aria-label="候補者一覧"></button>
`,'auditionHub')
}

function openSkyPanel(type){state.skyPanel=type;render()}
function closeSkyPanel(){state.skyPanel=null;render()}
function skyPanelContent(){
 const g=currentGroup();
 if(!state.skyPanel)return '';
 const head=`<div class="sky-sheet-head"><strong>${state.skyPanel==='actions'?'今週の経営判断':state.skyPanel==='studio'?'楽曲制作':state.skyPanel==='agency'?'事務所運営':state.skyPanel==='mission'?'目標と進行':'メニュー'}</strong><button onclick="closeSkyPanel()">✕</button></div>`;
 let body='';
 if(state.skyPanel==='actions'){
   body=`<div class="sky-action-grid">${ACTIONS.map(a=>`<button class="${g.selectedAction===a.id?'active':''}" onclick="selectAction('${a.id}')"><span>${a.icon}</span><strong>${a.name}</strong><small>${a.desc}<br>${a.impact}<br>費用 ${money(a.cost)}</small></button>`).join('')}</div><button class="sky-primary" onclick="executeWeek()" ${!g.selectedAction?'disabled':''}>今週を進める →</button>`;
 }else if(state.skyPanel==='studio'){
   body=studioTab();
 }else if(state.skyPanel==='agency'){
   body=agencyManagementCompact();
 }else if(state.skyPanel==='mission'){
   const p=clamp(Math.round(g.mission.current/g.mission.target*100),0,100);
   body=`<div class="sky-mission-detail"><h2>${g.mission.title}</h2><p>${g.mission.current.toLocaleString()} / ${g.mission.target.toLocaleString()}人</p><div class="sky-progress"><i style="width:${p}%"></i></div><p>残り ${g.mission.months}か月</p><button onclick="setTab('live');closeSkyPanel()">ライブ画面へ</button></div>`;
 }else{
   body=`<div class="sky-menu-grid">
     <button onclick="openSkyPanel('actions')">🎛️<strong>今週の経営判断</strong></button>
     <button onclick="openSkyPanel('studio')">♫<strong>楽曲制作</strong></button>
     <button onclick="openSkyPanel('agency')">🏢<strong>事務所運営</strong></button>
     <button onclick="save()">💾<strong>セーブ</strong></button>
     <button onclick="reset()">↺<strong>タイトルへ</strong></button>
   </div>`;
 }
 return `<div class="sky-sheet-backdrop" onclick="if(event.target===this)closeSkyPanel()"><div class="sky-sheet">${head}<div class="sky-sheet-body">${body}</div></div></div>`
}
function agencyManagementCompact(){
 const next=OFFICE_LEVELS.find(x=>x.level===state.officeLevel+1);
 return `<div class="sky-management">
  <div class="sky-stats"><div><span>資金</span><strong>${money(state.cash)}</strong></div><div><span>評判</span><strong>${state.reputation}</strong></div><div><span>所属枠</span><strong>${state.groups.length}/${office().slots}</strong></div></div>
  <h3>所属グループ</h3>${state.groups.map(g=>`<div class="sky-row"><div><strong>${g.name}</strong><small>Power ${groupPower(g)} / Fans ${g.fans.toLocaleString()}</small></div><button onclick="switchGroup('${g.id}');closeSkyPanel()">運営</button></div>`).join('')}
  <h3>スタッフ</h3><div class="sky-staff-grid">${STAFF.map(s=>`<div class="sky-staff"><strong>${s.name}</strong><small>${s.effect}</small>${hasStaff(s.id)?`<button onclick="fireStaff('${s.id}')">契約解除</button>`:`<button onclick="hire('${s.id}')">採用</button>`}</div>`).join('')}</div>
  ${next?`<div class="sky-upgrade"><strong>次の事務所：${next.name}</strong><small>必要評判 ${next.needRep} / ${money(next.cost)}</small><button onclick="upgradeOffice()">オフィス拡張</button></div>`:''}
 </div>`
}
function skyFrame(image,hotspots,active='board'){
 return `<div class="sky-page"><img class="sky-page-bg" src="assets/${image}" alt=""><div class="sky-hotspots">${hotspots}</div>${skyBottomNav(active)}</div>${skyPanelContent()}`
}
function skyBottomNav(active){
 ...
 return tabs.map((t,i)=>`<button aria-label="${t[1]}" class="sky-nav-hit ${active===t[0]?'active':''}" style="left:${xs[i]}%;width:20%" onclick="setTab('${t[0]}')"></button>`).join('')
}

function shell(content){
 return `<div class="app sky-app">${content}</div>${state.modalMemberId?memberModal(state.modalMemberId):''}${state.pendingStory?storyModal(state.pendingStory):''}`
}
...
function boardTab(){
 const g=currentGroup();
 return skyFrame('home.png',`
<button class="hot hit-menu" onclick="openSkyPanel('menu')" aria-label="メニュー"></button>
<button class="hot hit-feature" onclick="setTab('members')" aria-label="注目グループ"></button>
<button class="hot hit-feature-detail" onclick="setTab('members')" aria-label="グループ詳細"></button>
<button class="hot hit-mission" onclick="openSkyPanel('actions')" aria-label="今週のミッション"></button>
<button class="hot hit-consult" onclick="openMember('${members(currentGroup())[0]?.id||''}')" aria-label="相談"></button>
<button class="hot hit-main-goal" onclick="openSkyPanel('mission')" aria-label="メイン目標"></button>
<button class="hot hit-office-status" onclick="setTab('world')" aria-label="事務所状況"></button>
`,'board')
}
...
function membersTab(){
 return skyFrame('members.png',`
<button class="hot hit-menu" onclick="openSkyPanel('menu')" aria-label="メニュー"></button>
<button class="hot hit-center-profile" onclick="openMember('${center(currentGroup())?.id||''}')" aria-label="センタープロフィール"></button>
<button class="hot hit-member-grid" onclick="openMember('${members(currentGroup())[0]?.id||''}')" aria-label="メンバー一覧"></button>
<button class="hot hit-relations" onclick="openMember('${members(currentGroup())[1]?.id||members(currentGroup())[0]?.id||''}')" aria-label="メンバー関係"></button>
`,'members')
}
...
function liveTab(){
 return skyFrame('live.png',`
<button class="hot hit-menu" onclick="openSkyPanel('menu')" aria-label="メニュー"></button>
<button class="hot hit-live-setlist" onclick="openSkyPanel('studio')" aria-label="演出・編成"></button>
<button class="hot hit-live-goal" onclick="openSkyPanel('mission')" aria-label="目標詳細"></button>
<button class="hot hit-live-start" onclick="startLive()" aria-label="ライブ開始"></button>
`,'live')
}

function agencyTab(){
 return skyFrame('office.png',`
<button class="hot hit-menu" onclick="openSkyPanel('menu')" aria-label="メニュー"></button>
<button class="hot hit-office-overview" onclick="openSkyPanel('agency')" aria-label="事務所概要"></button>
<button class="hot hit-office-finance" onclick="openSkyPanel('agency')" aria-label="財務状況"></button>
<button class="hot hit-office-staff" onclick="openSkyPanel('agency')" aria-label="スタッフ"></button>
<button class="hot hit-office-brand" onclick="openSkyPanel('agency')" aria-label="ブランド"></button>
<button class="hot hit-office-upgrade" onclick="upgradeOffice()" aria-label="事務所改装"></button>
<button class="hot hit-office-project" onclick="openSkyPanel('studio')" aria-label="プロジェクト"></button>
`,'world')
}
